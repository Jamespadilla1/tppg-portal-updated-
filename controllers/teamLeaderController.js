...
const getTeamLeaders = async (req, res) => {
  try {
    ...
    let query = supabase.from('team_leaders').select('id, name, email, phone, unit_manager_id, sales_manager_id, status, created_at').order('created_at', { ascending: false });
...
const createTeamLeader = async (req, res) => {
  try {
    ...
    }]).select('id, name, email, phone, unit_manager_id, sales_manager_id, status, created_at').single();
    if (error) throw error;
...
const updateTeamLeader = async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('team_leaders').update(updateData).eq('id', req.params.id).select('id, name, email, phone, unit_manager_id, sales_manager_id, status, created_at').single();
    if (error) throw error;
